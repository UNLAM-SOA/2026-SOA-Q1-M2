...
tipo_evento_t verificarLluvia() {
  ...
  if (valor_actual > (LLUVIA_MIN_VALIDA + 100) && lluvia == false && digitalRead(GPIO_FC_C) == LOW) {
...
void callback(char *topic, byte *payload, unsigned int length) {
  ...
  if (topicStr == TOPIC_MODO) {
    ...
    if (mensaje == "ABRIR") {
      ...
      // solo abro si todavia no llego al final de carrera abierto
      if (digitalRead(GPIO_FC_A) == LOW) {

        evento = EVT_ABRIR_M;
        Serial.print("[COMANDO VIA APP] ABRIENDO VENTANA...");
        xQueueSend(queueEvents, &evento, NO_WAIT);

      } else {

        Serial.println("[COMANDO VIA APP] La ventana ya está abierta.");
        if(client.connected()){
          client.publish(TOPIC_NOTIF, "La ventana ya abierta");
        }
      }
    }
      ...
      // solo cierro si todavia no llego al final de carrera cerrado
      if (digitalRead(GPIO_FC_C) == LOW) {

        evento = EVT_CERRAR_M;
        Serial.print("[COMANDO VIA APP] CERRANDO VENTANA...");
        xQueueSend(queueEvents, &evento, NO_WAIT);

      } else {

        Serial.println("[COMANDO VIA APP] La ventana ya está cerrada.");
        if(client.connected()){
          client.publish(TOPIC_NOTIF, "La ventana ya cerrada");
        }
      }
    }
  }
